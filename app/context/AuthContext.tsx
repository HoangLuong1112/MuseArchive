...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    // Mock user: điều chỉnh nếu Account có nhiều field hơn
    const mockUser = {
        id: 'offline',
        userName: 'dev',
    ...
    // Load user và token từ localStorage khi mở lại trang (hoặc bypass)
    useEffect(() => {
        if (BYPASS) {
            // Bypass mode: set mock user và dummy tokens (cho frontend chạy)
        ...
        if (storedUser && access && refresh) {
            try {
                ...
                // nếu parse lỗi thì logout để reset
    ...
    // Lưu vào localStorage khi có thay đổi currentUser
    useEffect(() => {
        if (currentUser) {
            try {
                localStorage.setItem('currentUser', JSON.stringify(currentUser));
            } catch (e) {
                console.error('Failed to stringify currentUser:', e);
            }
        } else {
            localStorage.removeItem('currentUser');
        }
    ...
    const getAccessToken = async (): Promise<string | null> => {
        if (BYPASS) {
            // Trong chế độ bypass trả về dummy token mà không gọi backend
            return localStorage.getItem('accessToken') ?? 'dummy-access-token';
        }
